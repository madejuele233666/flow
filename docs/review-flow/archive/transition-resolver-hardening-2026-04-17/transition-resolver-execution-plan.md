...
- initial working 必须 `spawn_initial_working`
...
- 当前 step 是否完成
- reviewer-derived routing summary
...
- review 结束后，resolver 必须明确给出是：
  ...
  - enter challenger
  - wait for user
- repair routing 必须来自 reviewer-derived normalized result，不能把
  "主进程想再想想" 当成 blocker
- `reroute_class` 必须足够具体，避免 caller 再次读取 raw findings 自行脑补
...
- route to `return_caller_flow` with docs-first repair responsibility
- route to `return_caller_flow` with source-first repair responsibility
...
- `source_first` 是否允许 implementation auto-fix，必须由 phase 与 finding
  属性共同决定
- `reroute target` 必须由 normalized reviewer result 驱动，而不是由 caller
  自己重新解析 findings 文本
- 未被 shared consumer contract 接入的新 routing enum 不得先被宣称为
...
- `failed_challenger_verifier_evidence_path`
- `reopen_record_path`
...
- `challenger reopen` 必须显式要求写 machine-readable `reopen-record.json`
  并绑定 failed challenger refs
- `challenger reopen` 必须显式绑定被晋升为 working 的 challenger
  `agent_id`
...
- challenger 返回 findings 时，结果不是 `deny close and stop`，而是
  `record challenger_reopen`，并把 failed challenger session 晋升为下一次
  working baseline
- closure 之前必须已知 `review_run_dir`，并且 guard result 已显式记录为通过
...
- `review_run_dir` 缺失或 guard result 未记录时，resolver 必须阻止 closure
...
- `predecessor`
  ...
  - `evidence.agent_id`
  - `evidence.review_goal`
  - `evidence.review_pass_type`
  - `evidence.coverage_status`
  - `evidence.saturation_status`
- `reopen`
  - `promoted_working_agent_id`
...
1. initial working requested but no `spawn-decision.json` output is required
2. challenger requested but predecessor `source_review` binding is missing
3. challenger requested but predecessor findings are non-empty
4. challenger requested but predecessor `final_assessment` is not `pass`
5. challenger requested but predecessor evidence is not a `working` pass
6. challenger requested but predecessor `review_goal` is not
   `implementation_correctness`
7. challenger requested but predecessor `coverage_status` is not `complete`
8. challenger requested but predecessor `saturation_status` is not `exhaustive`
9. challenger requested but predecessor evidence `agent_id` does not match
   `source_review.agent_id`
10. challenger reopen requested but failed challenger refs or `reopen_record_path`
   are missing
11. challenger reopen requested but `promoted_working_agent_id` is missing or
    does not match the failed challenger `agent_id`
12. close requested but final pass is not challenger
13. close requested but final findings `final_assessment` is not `pass`
14. close requested but `closure_authority` is not
   `challenger_confirmed`
15. ordinary rerun requested but no active/resumable working session exists and
   no exception reason is recorded
16. built-in subagent required but caller tries to replace it with shell/`exec`
17. required verifier paths are missing or subject-incoherent
18. pause flags are absent, but the workflow tries to stop after preparatory
   artifacts only
19. docs-first review tries to route directly into implementation auto-fix
20. closure requested before the mandatory run-dir mechanical gate succeeds
...
openspec/schemas/modules/review-loop/
  ...
  transition-resolver/
    README.md
    contracts/
      transition-resolver-decision-v1.json
      transition-resolver-routing-v1.json
    bin/
      transition_resolver_validate.py
```
...
- normalized challenger-entry predecessor checks
  - `findings_final_assessment`
  - `evidence.agent_id`
  - `evidence.review_goal`
  - `evidence.review_pass_type`
...
- normalized mechanical gate metadata
  - `guard_command`
  - `mechanical_gate_status`
  - `guard_checked_at`
- exception reason when claiming a fresh working session is necessary

## Authoritative V1 Resolver Outputs

The authoritative machine-readable contracts now live under:
...
This section remains the design explanation for those contracts.

The authoritative v1 output is required to carry the earlier decision
metadata:

- `decision`
  - canonical transition enum from the earlier decision surface
  - `spawn_initial_working`
  - `send_input_same_working`
  - `resume_same_working`
  - `spawn_fresh_challenger`
...
- `decision_class`
  ...
  - `deny`
- `reason_code`
- `required_session_mode`: initial working, reuse working, fresh challenger,
  fresh working exception, or no-spawn
- `required_invocation_mode`
- `reroute_destination`
...
- `required_output_artifacts` must explicitly name `spawn-decision.json` for
  `spawn_initial_working`
- `required_output_artifacts` must explicitly name `spawn-decision.json` for
  `spawn_fresh_challenger` and `spawn_exception_working`
- `required_output_artifacts` must be able to name `reopen-record.json` when
  `record_challenger_reopen` is selected
- `authorized`
- `blocking`
- `required_mechanical_gate`
- `mechanical_gate_status`
...
Before any new routing enum becomes authoritative, it must be added to the
versioned routing contract and to every caller/consumer that executes resolver
outputs.
