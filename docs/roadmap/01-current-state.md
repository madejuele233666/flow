...
### 9. Gate A 的日用闭环已经正式文档化并完成验证

基于 `docs/day-use/task-flow-contract.md`、`docs/day-use/single-machine-baseline.md`、`docs/day-use/operator-runbook.md`、`docs/day-use/gate-a-smoke.md`，以及对应的实现和验证记录，当前可以确认：

- Gate A 的 canonical task-flow contract 已经 repo-owned 化。
- `HUD_DATA_DIR -> hud_config.toml` 的配置源规则已经显式化，并在前端配置加载路径中被验证。
- backend smoke、frontend smoke、runbook-from-zero、offline-to-recovery drill 已经形成可重复的验证材料。
- `docs/roadmap/11-horizon-a-day-use.md` 现在更多承担阶段回溯和后续阶段前置引用作用，而不是补文档的待办状态。
