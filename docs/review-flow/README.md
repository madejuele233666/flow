...
8. Keep the same working `agent_id` across ordinary reruns.
9. If the working `agent_id` changes, treat it as an exception-only recovery
   path and require an explicit machine-readable `spawn-decision.json`.
10. Continue until the latest working pass records:
   - `final_assessment=pass`
   - empty findings
   - `coverage_status=complete`
   - `saturation_status=exhaustive`
10. Validate the prior working outputs.
11. Start a fresh `challenger` session only after that validation succeeds.
12. Close only if the challenger returns zero findings with complete coverage.
