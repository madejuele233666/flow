...
5.  **编辑 `openspec/project.md`（英文）**：在项目级别的全局上下文中写入高层方向性准则（Architecture-First Principle、Alignment Protocol、AI Self-Verification 等），使所有 AI 在执行任何 openspec 变更时都能自动获取这些元规则作为背景知识。
6.  **编辑 `openspec/schemas/` 工作流定义**：在现有的 `spec-driven` 标准工作流的基础上，在 `design` 和 `tasks` artifact 的 `instruction` 中注入 AI 自我检验环节。具体来说：
    *   `design` artifact 的指令中增加：产出设计文档后，AI 必须自行执行架构审计检查清单并附上覆盖报告；当涉及参照系统时必须执行对标法 SOP。
    *   `tasks` artifact 的指令中增加：高危核心步骤必须插入 `【检查点】` 阻断；纯逻辑层文件必须插入 `【防腐规定】`。
    *   保留原有工作流的全部内容不变，仅在指令末尾追加自检规则。
...
- `project-level-guardrails`: 在 `openspec/project.md` 中设立项目级 AI 协作准则，所有变更自动继承。
- `schema-workflow-self-verification`: 在 `openspec/schemas/` 的工作流定义中注入 AI 自检环节，使检查清单和防御机制成为 artifact 生成流水线的内置步骤而非外挂要求。
...
- **工作流层**：通过编辑 `openspec/schemas/` 中的 schema `instruction`，将 AI 自检机制以最基础的方式固化进 artifact 生成流水线。这意味着任何使用此 schema 创建的变更，AI 在生成 `design.md` 和 `tasks.md` 时都会自动触发自检流程——无需依赖 skill 记忆或人工提醒。
- **项目层**：通过编辑 `openspec/project.md`，将高层方向性准则（如 Architecture-First Principle）注入全局上下文，使 AI 在处理任何变更时都能感知这些元规则。
- **文档层**：在 `docs/` 下创建详细的白皮书 `ai_architecture_guidelines.md`，包含 5 种反模式分类、7 项 Before/After 差距矩阵、对标法 SOP、20 项审计检查清单。
- **知识资产**：Before/After 差距矩阵作为案例教材永久存档。
