# Capability: IPC Protocol V2 Contract

## Purpose
Define one canonical IPC V2 wire contract shared by frontend and backend so both sides can develop independently while remaining interoperable.
...
These values are communicated in hello response and MUST be applied by peers according to key semantics. Pre-handshake enforcement MAY use an implementation-local safety size and does not depend on negotiated values.
